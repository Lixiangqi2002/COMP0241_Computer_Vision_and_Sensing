...
def choose_dataset():
    ...
    print("3. Custom path")
    print("4. Half of the Custom Dataset Path")
    
    choice = input("Enter choice (1-4): ")
    
    if choice == '1':
        return "fountain", "Fountain/images", "https://cvg-data.inf.ethz.ch/local-feature-evaluation-schoenberger2017/Strecha-Fountain.zip", Path(__file__).resolve().parent / "example"
    elif choice == '2':
        return "south-building", "south-building/images", "https://demuc.de/colmap/datasets/south-building.zip", Path(__file__).resolve().parent / "example"
    elif choice == '3':
        custom_path = "example/G10_dataset/images_firehy/"
        folder_name = "images"
        return folder_name, custom_path, None, Path(__file__).resolve().parent
    elif choice == '4':
        custom_path = "example/G10_dataset/images_firehy_half/"
        folder_name = "images"
        return folder_name, custom_path, None, Path(__file__).resolve().parent
    else:
...
def run():
    ...
    dataset_name, image_dir, data_url, base_path = choose_dataset()
    ...
    sift_extraction_options.num_threads = 4
    sift_extraction_options.max_image_size = 640
    sift_extraction_options.max_num_features = 2000  # Increased to detect more features
    ...
    # print(f"SIFT Processing time: {end_feature-start_feature}")
    if sfm_path.exists():
